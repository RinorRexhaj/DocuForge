...
PROB_TEXT_OVERLAY = 0.7        # Increased from 0.45 to make text overlays more frequent
PROB_COPY_PASTE = 0.8          # Increased from 0.6 for more visible copy-paste artifacts
PROB_SEAMLESS = 0.2            # Decreased from 0.4 to make splicing more obvious (less blending)
PROB_SIGNATURE_PASTE = 0.5     # Increased from 0.25 for more signature/photo forgeries
PROB_LOCAL_BLUR = 0.7          # Increased from 0.5 for more blur artifacts
PROB_COMPRESSION_NOISE = 0.8   # Increased from 0.6 for more compression artifacts

MAX_PATCH_AREA = 0.15          # Increased from 0.08 to make copy-paste patches larger and more visible
MIN_PATCH_AREA = 0.01          # Increased from 0.005 for more noticeable patches
...
def add_jpeg_noise(pil_img, quality_range=(20, 70)):  # Reduced quality range from (30, 95) for more artifacts
...
def add_gaussian_noise(pil_img, sigma=15):  # Increased default from 10 for more noise
...
def random_brightness_contrast(pil_img, bright_range=(0.7, 1.4), contrast_range=(0.7, 1.4)):  # Wider range from (0.8, 1.3)
    img = pil_img
...
def text_overlay(pil_img):
    ...
    # choose font - LARGER for more prominence
    font_size = max(16, int(min(w, h) * random.uniform(0.03, 0.07)))  # Increased size range
    try:
        font = ImageFont.truetype(FONT_PATH or "arial.ttf", font_size)
    except Exception:
        font = ImageFont.load_default()

    # choose random position and text - MORE VARIED
    text_candidates = ["EDITED", "Name: John Doe", "CONFIDENTIAL", "01/01/2000", "Amount: $999.99", 
                      "MODIFIED", "ALTERED", "Date Changed", "FAKE", "TAMPERED"]
    text = random.choice(text_candidates)

    # try to find an area to place text: random bounding box - LARGER
    box_w = int(w * random.uniform(0.2, 0.6))  # Increased from (0.15, 0.5)
    box_h = int(h * random.uniform(0.05, 0.15))  # Increased from (0.03, 0.12)
    ...
    if random.random() < 0.6:  # Increased probability of white patch overlay
        # overlay a white patch then write text (simulate replacing)
        draw.rectangle([x, y, x + box_w, y + box_h], fill=(255, 255, 255))
        text_color = (random.randint(0, 30), random.randint(0, 30), random.randint(0, 30))  # Darker text
    else:
        text_color = (200, 0, 0)  # More vibrant red for edited text
...
def copy_paste_splice(pil_img, seamless_prob=PROB_SEAMLESS):
    ...
    if random.random() < seamless_prob:
        ...
        # simple paste with MORE OPACITY for visibility
        alpha = random.uniform(0.95, 1.0)  # Increased from (0.85, 1.0) to make patches more opaque
...
def signature_photo_paste(pil_img):
    ...
    # choose small patch - LARGER for more visibility
    patch_area = int(h * w * random.uniform(0.01, 0.04))  # Increased from (0.005, 0.02)
    patch_w = max(30, int(np.sqrt(patch_area)))  # Increased from 20
    patch_h = max(20, int(patch_area / max(1, patch_w)))  # Increased from 10

    sx = random.randint(0, w - patch_w)
    sy = random.randint(0, h - patch_h)
    src = img_cv[sy:sy + patch_h, sx:sx + patch_w].copy()

    # apply transforms: rotate, scale, perspective - MORE DRAMATIC
    angle = random.uniform(-30, 30)  # Increased rotation range from (-20, 20)
    scale = random.uniform(0.7, 1.4)  # More varied scaling from (0.8, 1.2)
    ...
    # perspective warp - MORE AGGRESSIVE
    if random.random() < 0.8:  # Increased probability from 0.6
        pts1 = np.float32([[0, 0], [patch_w - 1, 0], [0, patch_h - 1], [patch_w - 1, patch_h - 1]])
        delta = patch_w * 0.25  # Increased distortion from 0.15
    ...
    # optional blur to simulate signature ink bleed or softness - LESS BLUR to keep edges visible
    if random.random() < 0.3:  # Decreased probability from 0.5
        k = random.choice([3,5])
...
def local_blur_smudge(pil_img):
    ...
    # LARGER blur regions for more visibility
    patch_w = int(w * random.uniform(0.1, 0.4))  # Increased from (0.05, 0.3)
    patch_h = int(h * random.uniform(0.08, 0.35))  # Increased from (0.03, 0.25)
    x = random.randint(0, w - patch_w)
    y = random.randint(0, h - patch_h)

    # sometimes motion blur - STRONGER BLUR
    sub = img_cv[y:y + patch_h, x:x + patch_w].copy()
    if random.random() < 0.4:
        # gaussian blur - STRONGER
        k = random.choice([9, 15, 21, 31])  # Increased from [5, 9, 15]
        sub = cv2.GaussianBlur(sub, (k, k), 0)
    else:
        # motion blur: kernel - STRONGER
        size = random.choice([15, 25, 35])  # Increased from [10, 15, 25]
...
def make_forgery(pil_img):
    ...
    # compress / noise - STRONGER EFFECTS
    if random.random() < PROB_COMPRESSION_NOISE:
        img = add_jpeg_noise(img, quality_range=(20, 60))  # Lower quality range from (30, 85)
        img = add_gaussian_noise(img, sigma=random.uniform(8, 25))  # Increased noise from (3, 16)
